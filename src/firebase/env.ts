...
export function withFrontendEnv(
  ...
  return configured.withExec([
    ...
    [
      ...
      "  ",
      "  const raw = input.substring(start, end + 1);",
      "",
      "  // Strip comments and trailing commas for more lenient parsing",
      "  const cleaned = raw",
      "    .replace(/\\/\\*[\\s\\S]*?\\*\\/|([^:]|^)\\/\\/.*$/gm, '$1')",
      "    .trim();",
      "",
      ...
      "    } catch (e) {",
      "      console.error('Failed to parse config. Cleaned input:', cleaned);",
      "      throw e;",
      "    }",
      "  }",
      "}",
