...
function withWifAuth(
  ...
  const resolvedAudience = audience?.trim() || `//iam.googleapis.com/${trimmedProvider}`;
  const credentials = JSON.stringify(
    {
      ...
      service_account_impersonation_url:
        `https://iamcredentials.googleapis.com/v1/projects/-/serviceAccounts/${trimmedServiceAccount}:generateAccessToken`,
      credential_source: {
...
function withResolvedApphostingAction(
  ...
  const deployDir = options.firebaseDir?.trim()
  ...
  return container
    ...
    .withEnvVariable("APPHOSTING_BACKEND_ID", backendId)
    .withEnvVariable("APPHOSTING_SKIP_DEPLOY", options.skipDeploy ? "true" : "false")
    .withEnvVariable(
...
export async function firebaseDeployApphostingPipeline(
  ...
  const installTarget = deletePreviewBackend
    ? undefined
  ...
  container = withResolvedApphostingAction(container, projectId, backendId, options);
