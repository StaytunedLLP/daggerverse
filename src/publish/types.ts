...
export interface HourlyReleaseResult {
   ...
   * set.
   *
   * `repair-dispatched` means the manifest was bumped previously but the
   * tag never appeared. Publish was dispatched for the *current* version
   * rather than bumping past it, which would strand that version permanently.
   */
  outcome:
    | "skipped-in-flight"
    | "nothing-to-release"
