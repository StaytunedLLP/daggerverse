...
export const createIssue = (
    ...
    const titleFile = join(tmpdir(), `issue_title_${Date.now()}.txt`);
    writeFileSync(tempFile, body);
    writeFileSync(titleFile, title);

    try {
        const ghRes = execGh(
            `api -X POST /repos/${owner}/${repo}/issues --field title=@${titleFile} --field body=@${tempFile}`,
            dryRun
        );
...
export const updateIssueTitle = (
    ...
    const tempFile = join(tmpdir(), `issue_title_update_${Date.now()}.txt`);
    writeFileSync(tempFile, title);
    try {
        return pipe(
            execGh(`issue edit ${number} --title "$(cat ${tempFile})"`, dryRun),
            map(() => undefined)
        );
    } finally {
        try { unlinkSync(tempFile); } catch { }
...
export const createPullRequest = (
    ...
    const titleFile = join(tmpdir(), `pr_title_${Date.now()}.txt`);
    writeFileSync(tempFile, body);
    writeFileSync(titleFile, title);
    try {
        return pipe(
            execGh(
                `pr create --title "$(cat ${titleFile})" --body-file "${tempFile}" --base ${base} --head ${head}`,
                dryRun
            ),
            map(() => undefined)
        );
