...
const main = async (): Promise<Result<EffectError | { _tag: "LogicError"; message: string }, void>> => {
    ...
    const prodEnv = Object.keys(FIREBASE_PROJECTS).find(env => FIREBASE_PROJECTS[env].isProduction);
    if (!prodEnv) {
        return failure({ _tag: "LogicError", message: "No production environment found in configuration" });
    }
    const prodConfig = FIREBASE_PROJECTS[prodEnv];
