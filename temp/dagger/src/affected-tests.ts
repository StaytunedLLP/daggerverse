...
export const internalAffectedSelectorCommand =
  "__internal_affected_selectors__";
...
const runtimeScriptUrl = new URL(
  "./affected-selectors.runtime.ts",
  (import.meta as ImportMeta & { url: string }).url,
);
const internalAffectedSelectorProgram = readFileSync(runtimeScriptUrl, "utf8");
...
/**
 * Builds a bash script that executes an internal Node program to compute affected selectors.
 *
 ...
 */
export function buildInternalSelectorProgram(
  base: string,
  selectorMode: string,
): string {
